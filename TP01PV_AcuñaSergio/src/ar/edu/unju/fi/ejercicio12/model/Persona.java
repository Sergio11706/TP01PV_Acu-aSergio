...
public class Persona {
	...
	public int CalcularEdad() {
		...
		if( fecha.get(Calendar.MONTH) > hoy.get(Calendar.MONTH) ) return edad-1;
		if( fecha.get(Calendar.MONTH) < hoy.get(Calendar.MONTH) ) return edad;
		if( fecha.get(Calendar.DAY_OF_MONTH) < hoy.get(Calendar.DAY_OF_MONTH) ) return edad;
		return edad-1;
	}
	...
	public String ObtenerEstacion() {
		...
		if(fecha.before(otoño)) return "Verano";
		else {
			if(fecha.before(invierno)) return "Otoño";
			else {
				if(fecha.before(primavera)) return "Invierno";
				else {
					if(fecha.before(verano)) return "Primavera";
					else return "Verano";
				}
			}
		}
	...
	public void MostrarDatos() {
		...
		System.out.println("Fecha de Nacimiento: "+fecha.get(Calendar.DAY_OF_MONTH)+"/"+(fecha.get(Calendar.MONTH)+1)+"/"+fecha.get(Calendar.YEAR));
